...
function NoWorkspacesAssigned() {
  ...
  return (
    <div
      style={{ height: isMobile ? "100%" : "calc(100% - 32px)" }}
      className="transition-all duration-500 relative md:ml-[2px] md:mr-[16px] md:my-[16px] md:rounded-[16px] bg-zinc-900 light:bg-white w-full h-full overflow-hidden flex flex-col items-center justify-center p-6"
    >
      {isMobile && <SidebarMobileHeader />}
      <div className="flex flex-col items-center max-w-md text-center p-8 rounded-2xl bg-white/[0.03] light:bg-slate-50 border border-white/10 light:border-slate-200 shadow-2xl">
        <div className="w-14 h-14 rounded-2xl bg-[#2563EB]/10 flex items-center justify-center mb-5 text-[#3B82F6]">
          <Plus size={28} weight="bold" />
        </div>
        <h2 className="text-2xl font-bold text-white light:text-slate-900 mb-2">
          Welcome to ContextIQ
        </h2>
        <p className="text-sm text-[#94A3B8] light:text-[#475569] mb-6 leading-relaxed">
          Create your first private workspace to organize documents and start
          asking questions.
        </p>
        <button
          type="button"
          onClick={showModal}
          className="w-full flex items-center justify-center gap-2 py-3 px-5 rounded-xl bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-medium text-sm transition-all duration-200 shadow-lg shadow-blue-500/20 cursor-pointer"
        >
          <Plus size={18} weight="bold" />
          <span>Create Workspace</span>
        </button>
        <p className="text-xs text-[#64748B] light:text-[#94A3B8] mt-4">
          Your workspaces and knowledge are private to you.
        </p>
      </div>
