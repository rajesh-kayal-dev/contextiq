...
const TABS = {
  ...
  members: Members,
  agent: WorkspaceAgentConfiguration,
...
function ShowWorkspaceChat() {
  ...
  const TabContent = TABS[tab] || GeneralAppearance;
  return (
    <div className="w-screen h-screen overflow-hidden bg-zinc-950 light:bg-slate-50 flex">
      ...
      >
        <div className="flex gap-x-10 pt-6 pb-4 ml-16 mr-8 border-b-2 border-white light:border-theme-chat-input-border border-opacity-10">
          ...
          <TabItem
            title={t("workspaces-settings.general", "General")}
            icon={<Wrench className="h-6 w-6" />}
            to={paths.workspace.settings.generalAppearance(slug)}
            activeTabs={["general", "general-appearance"]}
          />
          <TabItem
            title={t("workspaces-settings.chat", "Chat")}
            icon={<ChatText className="h-6 w-6" />}
            to={paths.workspace.settings.chatSettings(slug)}
            activeTabs={["chat", "chat-settings"]}
          />
          <TabItem
            title={t("workspaces-settings.vector", "Knowledge")}
            icon={<Database className="h-6 w-6" />}
            to={paths.workspace.settings.vectorDatabase(slug)}
            activeTabs={["vector", "vector-database"]}
          />
          <TabItem
            title={t("workspaces-settings.members", "Members")}
          ...
          />
          <TabItem
            title={t("workspaces-settings.agent", "Agent")}
            icon={<Robot className="h-6 w-6" />}
            to={paths.workspace.settings.agentConfig(slug)}
            activeTabs={["agent", "agent-config"]}
          />
        </div>
...
function TabItem({ title, icon, to, activeTabs = [], visible = true }) {
  const { tab } = useParams();
