...
import UserApiKeyModal from "@/components/Modals/UserApiKeyModal";
...
export default function ChatContainer({
  ...
  const [requiredProvider, setRequiredProvider] = useState(
    workspace?.chatProvider || "openai"
  );

  useEffect(() => {
    function handleOpenApiKeyModal(e) {
      const provider =
        e?.detail?.provider || workspace?.chatProvider || "openai";
      setRequiredProvider(provider);
      setApiKeyModalOpen(true);
    }
    window.addEventListener("open-api-key-modal", handleOpenApiKeyModal);
    return () =>
      window.removeEventListener("open-api-key-modal", handleOpenApiKeyModal);
      ...
      <ChatSidebarProvider>
        ...
        >
          <ChatSettingsMenu
            ...
            threadSlug={activeThreadSlug}
          />
          <div className="flex-1 min-w-0 relative md:rounded-[16px] bg-[#111827] light:bg-[#FFFFFF] w-full h-full overflow-hidden border-none light:border-solid light:border light:border-[#CBD5E1]">
            <SidebarMobileHeader />
            <WorkspaceModelPicker workspaceSlug={workspace.slug} />
            <DnDFileUploaderWrapper>
              <div className="flex flex-col h-full w-full items-center justify-center px-4 py-6 overflow-y-auto no-scroll">
                <div className="flex flex-col items-center w-full max-w-[800px]">
                  {/* ContextIQ Empty State Hero */}
                  <div className="flex flex-col items-center mb-6">
                    <div className="w-16 h-16 mb-4 rounded-2xl bg-[#172033] light:bg-slate-100 border border-white/10 light:border-slate-200 flex items-center justify-center shadow-lg light:shadow-sm">
                      <img
                    ...
                    <h1 className="text-[#F8FAFC] light:text-[#0F172A] text-2xl md:text-3xl font-semibold mb-2 text-center tracking-tight">
                      Ask your knowledge anything.
                    </h1>
                    <p className="text-[#94A3B8] light:text-[#475569] text-sm md:text-base text-center max-w-md leading-relaxed">
                      Search across your documents and get answers with relevant
                      context.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap justify-center gap-2 w-full max-w-[600px] mb-4">
                    {[
                      {
                        label: "Summarize",
                      ...
                      },
                      {
                        label: "Explain",
                        prompt:
                          "Explain the most important concept from my documents.",
                      },
                      {
                        label: "Find Info",
                        prompt: "Find the most relevant information about ",
                      },
                    ].map(({ label, prompt }) => (
                      <button
                        key={label}
                        ...
                        className="flex items-center justify-center px-3 py-1.5 rounded-lg bg-[#172033]/60 light:bg-white border border-white/[0.08] light:border-[#CBD5E1] text-[#94A3B8] light:text-[#475569] text-xs md:text-sm font-medium hover:bg-[#2563EB]/15 light:hover:bg-blue-50 hover:border-[#2563EB]/60 light:hover:border-[#2563EB] hover:text-[#F8FAFC] light:hover:text-[#2563EB] transition-all duration-200"
                      >
                        {label}
                      </button>
                    ))}
    ...
    <ChatSidebarProvider>
      ...
      >
        <ChatSettingsMenu
          ...
          threadSlug={activeThreadSlug}
        />
        <div className="flex-1 min-w-0 relative md:rounded-[16px] bg-[#121A2B] light:bg-white text-white light:text-slate-900 h-full overflow-hidden border-none light:border-solid light:border light:border-theme-modal-border">
          <SidebarMobileHeader />
          <WorkspaceModelPicker workspaceSlug={workspace.slug} />
          <DnDFileUploaderWrapper>
        ...
        <UserApiKeyModal
          ...
          hideModal={() => setApiKeyModalOpen(false)}
