...
const System = {
  cacheKeys: {
    footerIcons: "contextiq_footer_links",
    supportEmail: "contextiq_support_email",
    customAppName: "contextiq_custom_app_name",
    canViewChatHistory: "contextiq_can_view_chat_history",
    deploymentVersion: "contextiq_deployment_version",
  },
  ping: async function () {
  ...
  registerUser: async function (body) {
    return await fetch(`${API_BASE}/request-token/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...body }),
    })
  ...
  lookupGuestPin: async function (body) {
    return await fetch(`${API_BASE}/request-token/guest/lookup`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...body }),
    })
      ...
      .then((res) => res)
      .catch((e) => {
        return { isExisting: false, message: e.message };
      });
  },
  setupGuest: async function (body) {
    return await fetch(`${API_BASE}/request-token/guest/setup`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...body }),
    })
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || "Guest setup failed.");
        return data;
      })
      .then((res) => res)
      .catch((e) => {
  ...
  loginGuest: async function (body) {
    return await fetch(`${API_BASE}/request-token/guest/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...body }),
    })
  ...
  updateGuestName: async function (body) {
    return await fetch(`${API_BASE}/request-token/guest/update-name`, {
      method: "POST",
      headers: baseHeaders(),
      body: JSON.stringify({ ...body }),
    })
  ...
  upgradeGuest: async function (body) {
    return await fetch(`${API_BASE}/request-token/guest/upgrade`, {
      method: "POST",
      headers: baseHeaders(),
      body: JSON.stringify({ ...body }),
    })
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || "Account upgrade failed.");
        return data;
      })
      .then((res) => res)
      .catch((e) => {
  ...
  recoverAccount: async function (username) {
    return await fetch(`${API_BASE}/system/recover-account`, {
      ...
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.message || data.error || "Error recovering account.");
        }
        return data;
      })
      .catch((e) => {
  ...
  resetPassword: async function (token, newPassword, confirmPassword) {
    return await fetch(`${API_BASE}/system/reset-password`, {
      ...
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.message || data.error || "Error resetting password.");
        }
        return data;
      })
      .catch((e) => {
  ...
  fetchEmbedConfig: async function () {
    return await fetch(`${API_BASE}/embed/config`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => res.json())
      .catch((e) => {
        console.error(e);
        return { config: null, workspaces: [], error: e.message };
      });
  },

  updateEmbedConfig: async function (config = {}) {
    return await fetch(`${API_BASE}/embed/config`, {
      method: "POST",
      headers: baseHeaders(),
