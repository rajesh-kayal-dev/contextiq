...
const Workspace = {
  ...
  update: async function (id = null, updates = {}) {
    ...
    // When switching to ContextIQ-router, chatModel is not used.
    // When switching away from ContextIQ-router, clear router_id.
    if (validatedUpdates?.chatProvider === "ContextIQ-router") {
      validatedUpdates.chatModel = null;
    } else if (
      validatedUpdates?.chatProvider &&
      validatedUpdates.chatProvider !== "ContextIQ-router"
    ) {
  ...
  supportsNativeToolCalling: async function (workspace = {}) {
    try {
      ...
      if (provider === "ContextIQ-router") {
        ...
        const fallbackProvider = new AIbitat(
          fallbackConfig
        ).getProviderForConfig(fallbackConfig);
      ...
      const nativeToolCalling =
        await agentProvider?.supportsNativeToolCalling?.();
  ...
  syncWorkspaceOwnership: async function () {
    try {
      const unassignedWorkspaces = await prisma.workspaces.findMany({
        where: {
          workspace_users: {
            none: {},
          },
      ...
      if (unassignedWorkspaces.length === 0) return;
      ...
      for (const ws of unassignedWorkspaces) {
        await WorkspaceUser.create(firstUser.id, ws.id);
      }
      console.log(
        `[Ownership Sync] Assigned ${unassignedWorkspaces.length} unassigned workspaces to User ID ${firstUser.id}.`
      );
    } catch (err) {
      console.error("[Ownership Sync Failed]:", err.message);
    }
  },
};
