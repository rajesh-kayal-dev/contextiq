 ...
 */
async function generateImageForWorkspace({
  prompt,
  size,
  signal,
  user = null,
  ...
  const provider = await getImageGeneratorProvider(user);
 ...
 * @param {{prompt: string, images: Buffer[], size?: string, signal?: AbortSignal, user?: object}} params
 * @returns {Promise<{storageFilename: string, filename: string, fileSize: number, buffer: Buffer}>}
 */
async function editImageForWorkspace({
  prompt,
  images,
  size,
  signal,
