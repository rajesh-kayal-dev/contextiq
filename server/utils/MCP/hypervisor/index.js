...
/**
 * @class MCPHypervisor
 * @description A class that manages MCP servers found in the storage/plugins/ContextIQ_mcp_servers.json file.
 ...
 * For example, if the tools requires `npx` then the context in which ContextIQ mains process is running will need to access npx.
 * This is typically not common in our pre-built image so may not function. But this is the case anywhere MCP is used.
 *
 * ContextIQ will take care of porting MCP servers to agent-callable functions via @agent directive.
...
class MCPHypervisor {
  ...
  #setupConfigFile() {
    this.mcpServerJSONPath =
      process.env.NODE_ENV === "development"
        ? path.resolve(
            __dirname,
            `../../../storage/plugins/contextiq_mcp_servers.json`
          )
        : path.resolve(
            process.env.STORAGE_DIR ??
              path.resolve(__dirname, `../../../storage`),
            `plugins/contextiq_mcp_servers.json`
          );
    ...
    if (fs.existsSync(legacyPath) && !fs.existsSync(this.mcpServerJSONPath)) {
      try {
        fs.copyFileSync(legacyPath, this.mcpServerJSONPath);
      } catch (e) {
        this.log(`Failed to copy legacy mcp config: ${e.message}`);
      }
    }
  ...
  updateSuppressedTools(serverName, toolName, enabled) {
    ...
    if (!server.ContextIQ) server.ContextIQ = {};
    if (!Array.isArray(server.ContextIQ.suppressedTools))
      server.ContextIQ.suppressedTools = [];

    const suppressedTools = server.ContextIQ.suppressedTools;
    ...
    server.ContextIQ.suppressedTools = suppressedTools;
  ...
  getSuppressedTools(serverName) {
    ...
    return config?.server?.ContextIQ?.suppressedTools || [];
  }
  ...
  async #buildMCPServerENV(server) {
    ...
    if (
      process.env.CONTEXTIQ_RUNTIME === "docker" ||
      process.env.ContextIQ_RUNTIME === "docker"
    ) {
      baseEnv = {
  ...
  async bootMCPServers() {
    ...
    for (const { name, server } of serverDefinitions) {
      if (
        server.ContextIQ?.hasOwnProperty("autoStart") &&
        server.ContextIQ.autoStart === false
      ) {
        this.log(
          `MCP server ${name} has ContextIQ.autoStart property set to false, skipping boot!`
        );
        this.mcpLoadingResults[name] = {
          status: "failed",
          message: `MCP server ${name} has ContextIQ.autoStart property set to false, boot skipped!`,
        };
        continue;
      }
