...
function isClassificationOrInvalidModel(modelName = "") {
...
function sanitizeGroqModel(candidate) {
  if (!candidate || typeof candidate !== "string" || !candidate.trim()) {
    return DEFAULT_GROQ_MODEL;
  }
  if (isClassificationOrInvalidModel(candidate)) {
    return DEFAULT_GROQ_MODEL;
  }
...
class GroqLLM {
  constructor(embedder = null, modelPreference = null, apiKey = null) {
    const { OpenAI: OpenAIApi } = require("openai");
    const key = apiKey || process.env.GROQ_API_KEY;
    if (!key) throw new Error("API_KEY_REQUIRED:groq");
    ...
    this.model = sanitizeGroqModel(
      modelPreference || process.env.GROQ_MODEL_PREF || DEFAULT_GROQ_MODEL
    );
    this.limits = {
  ...
  async isValidChatCompletionModel(modelName = "") {
    ...
    return !isClassificationOrInvalidModel(modelName);
  }
  ...
  async getChatCompletion(messages = null, { temperature = 0.7 }) {
    if (!(await this.isValidChatCompletionModel(this.model))) {
      this.#log(
        `Model ${this.model} is not valid for chat completion. Falling back to ${DEFAULT_GROQ_MODEL}.`
      );
    ...
    try {
      ...
      const is404Or400 =
        ...
        (error?.message || "").includes("text classification");

      if (is404Or400 && this.model !== DEFAULT_GROQ_MODEL) {
        this.#log(
          `Model ${this.model} failed with ${error.status || error.message}. Retrying with fallback model ${DEFAULT_GROQ_MODEL}...`
        );
        this.model = DEFAULT_GROQ_MODEL;
        return await this.getChatCompletion(messages, { temperature });
      }
      throw error;
    }
  }
  ...
  async streamGetChatCompletion(messages = null, { temperature = 0.7 }) {
    if (!(await this.isValidChatCompletionModel(this.model))) {
      this.#log(
        `Model ${this.model} is not valid for live streaming. Falling back to ${DEFAULT_GROQ_MODEL}.`
      );
      this.model = DEFAULT_GROQ_MODEL;
    }
    ...
    try {
      ...
      if (is404Or400 && this.model !== DEFAULT_GROQ_MODEL) {
        ...
        this.model = DEFAULT_GROQ_MODEL;
        return await this.streamGetChatCompletion(messages, { temperature });
      }
      throw error;
    }
  }
