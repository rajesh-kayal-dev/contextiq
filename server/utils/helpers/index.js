 ...
 * @notice Use resolveProviderConnector instead as this function DOES NOT handle the ContextIQ-router provider.
 * You should only use this function if you are absolutely sure you are not using the ContextIQ-router provider ever in your code.
...
function getLLMProvider({ provider = null, model = null, apiKey = null } = {}) {
  ...
  switch (LLMSelection) {
    ...
    case "ContextIQ-router":
      // Model router is handled separately in stream.js via ContextIQModelRouter.
      // This case should not be hit directly - if it is, throw a descriptive error.
      throw new Error(
        "ContextIQ-router provider must be resolved via ContextIQModelRouter class, not getLLMProvider directly."
      );
    default:
      throw new Error(
 ...
 */
async function getImageGeneratorProvider(user = null) {
  const provider =
    process.env.IMAGE_GEN_PROVIDER ||
    (process.env.IMAGE_GEN_OPENAI_KEY || process.env.OPEN_AI_KEY
      ? "openai"
      : null);

  if (!provider) {
    throw new Error(
      "No image generation provider is configured. Please select an Image Generation provider (e.g. OpenAI DALL-E 3 or OpenRouter) in Settings → Image Generation."
    );
  }
  ...
  if (user?.id) {
    ...
    userApiKey = await UserApiKey.getKey({ userId: user.id, provider });
  }

  switch (provider) {
    case "openai":
      const { OpenAiImageGenerator } = require("../ImageGenerators/openAi");
      return new OpenAiImageGenerator(userApiKey);
    case "ollama":
    ...
    default:
      throw new Error(
        `Unsupported image generation provider "${provider}". Please configure a valid provider in Settings → Image Generation.`
      );
  }
}
...
function getLLMProviderClass({ provider = null } = {}) {
  switch (provider) {
    ...
    case "cerebras":
      ...
      return CerebrasLLM;
    case "ContextIQ-router":
      const { ContextIQModelRouter } = require("../AiProviders/modelRouter");
      return ContextIQModelRouter;
    default:
      return null;
  }
}
...
function getSystemProviderApiKey(provider = "") {
  ...
  switch (cleanProvider) {
    ...
    case "azure":
      return process.env.AZURE_OPENAI_KEY || null;
    default:
      return null;
  }
}

function isApiKeyRequiredProvider(provider = "") {
  const cleanProvider = String(provider || "")
    .trim()
    .toLowerCase();
  ...
  return !NO_KEY_PROVIDERS.includes(cleanProvider);
}

/**
 * Resolves the provider connector and routing context for a prompt execution.
 * @param {Object} opts
 * @param {Object} opts.workspace - Workspace model instance
 * @param {string} [opts.prompt] - User prompt
 * @param {Object|null} [opts.user] - Requesting user
 * @param {Object|null} [opts.thread] - Workspace thread
 * @param {Array} [opts.attachments] - Attachments array
 * @param {Array|null} [opts.chatHistoryOverride] - Override for chat history
...
async function resolveProviderConnector({
  ...
  // Per-User API key resolution with system fallback
  let userApiKey = null;
  if (user?.id) {
    const { UserApiKey } = require("../../models/userApiKey");
    userApiKey = await UserApiKey.getKey({
      userId: user.id,
  ...
  if (!userApiKey) {
    userApiKey = getSystemProviderApiKey(effectiveProvider);
  }

  if (!userApiKey && isApiKeyRequiredProvider(effectiveProvider)) {
    throw new Error(`API_KEY_REQUIRED:${effectiveProvider}`);
  }
  ...
  const router = new ContextIQModelRouter(routerWorkspace);
