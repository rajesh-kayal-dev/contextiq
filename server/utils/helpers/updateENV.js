...
async function validDockerizedUrl(input = "") {
  ...
  try {
    ...
    if (isPortAvailableFromDocker)
      return "Port is not running a reachable service on loopback address from inside the ContextIQ container. Please use host.docker.internal (for linux use 172.17.0.1), a real machine ip, or domain to connect to your service.";
  } catch (error) {
...
function dumpENV() {
  ...
  const protectedKeys = [
    ...
    "CONTEXTIQ_FETCH_TIMEOUT",
    "CONTEXTIQ_MAX_RETRIES",
    "ContextIQ_FETCH_TIMEOUT",
    "ContextIQ_MAX_RETRIES",
