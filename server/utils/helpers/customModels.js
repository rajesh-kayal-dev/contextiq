// Top-level provider requires removed for faster server startup.
// Provider helper functions lazy-require their respective modules on demand.
...
async function getGroqAiModels(_apiKey = null) {
  ...
  const models = (
    ...
    (model) =>
      ...
      !model.id.includes("tool-use") &&
      !model.id.includes("guard") &&
      !model.id.includes("safeguard") &&
      !model.id.includes("classifier")
  );
    ...
    .then((models) =>
      models
        .filter((model) => {
          ...
          const caps = model?.details?.families || [];
          if (
            name.includes("flux") ||
            ...
            return false;
          if (caps.includes("image")) return false;
          return true;
        })
        .map((model) => {
