 ...
 * the mime type for documents is `application/ContextIQ-document` - anything else is assumed to be an image.
...
async function processDocumentAttachments(attachments = []) {
  ...
  for (const attachment of attachments) {
    if (
      ...
      attachment.mime &&
      [
        "application/ContextIQ-document",
        "application/contextiq-document",
      ].includes(attachment.mime.toLowerCase())
    )
...
async function streamChat({
  ...
  try {
    ...
    console.error(`[ApiChatHandler Error]`, error);
    const status =
