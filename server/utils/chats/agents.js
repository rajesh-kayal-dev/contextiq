...
async function grepAgents({
  ...
  // Guest users default to Chat mode and must NOT run automatic agent flow unless @agent is explicitly invoked
  const shouldRunAgent =
    agentHandles.length > 0 ||
    (!isGuestUser && nativeToolingEnabled && workspace?.chatMode !== "chat");
