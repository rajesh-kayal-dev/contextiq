...
https://docs.ContextIQ.com/agent/intelligent-tool-selection
  ...
  async #safeProviderCall(providerCall) {
    try {
      ...
      const isToolError =
        ...
        error.message?.toLowerCase().includes("tool_calling") ||
        error.message?.toLowerCase().includes("tools is not supported") ||
        (error.message?.toLowerCase().includes("tools") &&
          error.message?.toLowerCase().includes("not supported"));

      if (isToolError) {
        throw new APIError(
          "This model does not support Agent tools. Switch to Chat mode or choose a tool-capable model."
        );
      }
