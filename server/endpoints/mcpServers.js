...
function mcpServersEndpoints(app) {
  ...
  app.get(
    ...
    async (_request, response) => {
      try {
        const servers = await getMCP().servers();
        return response.status(200).json({
          success: true,
          servers,
        });
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        const result = await getMCP().toggleServerStatus(name);
        return response.status(200).json({
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        const { name } = reqBody(request);
        const result = await getMCP().deleteServer(name);
        return response.status(200).json({
          success: result.success,
          error: result.error,
        });
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        const result = await getMCP().toggleToolSuppression(
          serverName,
          toolName,
          enabled
        );
